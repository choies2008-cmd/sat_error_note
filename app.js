...
const GEMINI_MODELS = [];
const DEFAULT_MODEL_FALLBACK = 'gemini-3.5-flash-lite';
const DEFAULT = { exams:[], records:[], settings:{theme:'system', afterSave:'next', geminiApiKey:'', geminiModel:DEFAULT_MODEL_FALLBACK} };
let db = loadDB();
...
function loadDB(){ try { const x=JSON.parse(localStorage.getItem(KEY)); return x && x.exams && x.records ? {...DEFAULT,...x,settings:{...DEFAULT.settings,...x.settings,geminiModel:x.settings?.geminiModel||DEFAULT.settings.geminiModel}} : structuredClone(DEFAULT); } catch { return structuredClone(DEFAULT); } }
...
function renderSettings(main){
  const hasKey=!!db.settings.geminiApiKey;
  const selectedModel=db.settings.geminiModel||'';
  const modelOptions=GEMINI_MODELS.length
    ? GEMINI_MODELS.map(m=>{
        const usable=(m.supportedGeneration||[]).includes('generateContent');
        ...
        return `<option value="${esc(id)}" ${selected} ${usable?'':'disabled'}>${esc(label)}${version}${usable?'':' · 이 앱의 생성 기능 미지원'}</option>`;
      }).join('')
    : `<option value="">API 키를 저장한 뒤 모델 목록을 불러오세요</option>`;
  const modelCount=GEMINI_MODELS.length;
  const usableCount=GEMINI_MODELS.filter(m=>(m.supportedGeneration||[]).includes('generateContent')).length;
  main.innerHTML=`<div class="settings-section"><div class="settings-title">AI 설정</div><div class="settings-card api-key-card">
    <div class="api-key-head"><div><strong>Gemini API 키</strong><small>Google AI Studio에서 발급받은 API 키를 입력하세요.</small></div><span class="api-key-status ${hasKey?'saved':''}">${hasKey?'저장됨':'미입력'}</span></div>
    <div class="api-key-row"><input id="geminiApiKeyInput" class="input" type="password" autocomplete="off" spellcheck="false" placeholder="AIza..." value="${esc(db.settings.geminiApiKey)}" /><button class="secondary-btn" type="button" onclick="toggleApiKeyVisibility()" id="apiKeyToggle">보기</button></div>
    <div class="api-key-actions"><button class="primary-btn" type="button" onclick="saveGeminiApiKey()">API 키 저장</button>${hasKey?'<button class="secondary-btn" type="button" onclick="clearGeminiApiKey()">삭제</button>':''}</div>
    <div class="field" style="margin-top:16px"><label class="field-label">AI 모델</label>
      <select class="select" id="geminiModelSelect" onchange="saveGeminiModel(this.value)">${modelOptions}</select>
      <div class="helper">Gemini API의 <code>/v1/models</code>에서 모델 목록을 가져옵니다. 전체 ${modelCount}개 · 이 앱에서 생성에 사용할 수 있는 모델 ${usableCount}개</div>
      <button class="secondary-btn full" style="margin-top:10px" type="button" onclick="loadGeminiModels(true)" ${hasKey?'':'disabled'}>↻ 모델 목록 새로고침</button>
    </div>
    <div class="helper">키와 선택한 모델은 이 브라우저의 localStorage에 저장됩니다. 백업 JSON에는 API 키가 포함되지 않습니다.</div>
    <div class="notice api-key-warning">⚠️ 이 방식은 개인용으로 간단하게 쓰기 위한 방식입니다. GitHub Pages의 웹앱에서는 브라우저에 저장된 API 키를 완전히 비공개로 숨길 수 없습니다.</div>
  </div></div>
  ...
  <div class="settings-section"><div class="settings-title">앱 정보</div><div class="settings-card"><div class="settings-btn"><span>버전<small>1.1.0 · Gemini 모델 자동 목록</small></span></div><div class="settings-btn"><span>데이터 저장 방식<small>이 기기의 브라우저 localStorage에 저장됩니다.</small></span></div><button class="settings-btn" onclick="resetData()"><span style="color:var(--danger)">모든 데이터 삭제<small>모의고사와 오답 기록을 모두 삭제합니다.</small></span><b>›</b></button></div></div>
  <input id="restoreInput" type="file" accept="application/json,.json" hidden onchange="restoreData(event)" />`;
}
async function loadGeminiModels(forceToast=false){
  const key=db.settings.geminiApiKey?.trim();
  if(!key){ if(forceToast) toast('먼저 Gemini API 키를 저장하세요.'); return; }
  try{
    const all=[];
    let pageToken='';
    do{
      const qs=new URLSearchParams({pageSize:'1000',key});
      if(pageToken) qs.set('pageToken',pageToken);
      const res=await fetch(`https://generativelanguage.googleapis.com/v1/models?${qs.toString()}`);
      const data=await res.json();
      if(!res.ok) throw new Error(data?.error?.message||`모델 목록 요청 실패 (${res.status})`);
      for(const m of (data.models||[])){
        all.push({
          name:m.name||'',
          baseModelId:m.baseModelId||String(m.name||'').replace(/^models\//,''),
          displayName:m.displayName||m.baseModelId||m.name||'',
          version:m.version||'',
          description:m.description||'',
          supportedGeneration:m.supportedGenerationMethods||[]
        });
      }
      pageToken=data.nextPageToken||'';
    }while(pageToken);
    GEMINI_MODELS.splice(0,GEMINI_MODELS.length,...all.sort((a,b)=>a.displayName.localeCompare(b.displayName)));
    const selected=GEMINI_MODELS.find(m=>(m.baseModelId||'')===db.settings.geminiModel);
    if(!selected || !(selected.supportedGeneration||[]).includes('generateContent')){
      const flashLite=GEMINI_MODELS.find(m=>/flash-lite/i.test(m.displayName||'') && (m.supportedGeneration||[]).includes('generateContent'));
      const first=flashLite || GEMINI_MODELS.find(m=>(m.supportedGeneration||[]).includes('generateContent'));
      if(first) { db.settings.geminiModel=first.baseModelId; saveDB(); }
    }
    if(forceToast) toast(`모델 ${GEMINI_MODELS.length}개를 불러왔습니다.`);
    render();
  }catch(err){ console.error(err); toast(`모델 목록 불러오기 실패: ${err.message||'알 수 없는 오류'}`); }
}

function setTheme(t){ db.settings.theme=t; saveDB(); render(); }
function saveGeminiApiKey(){ const input=document.getElementById('geminiApiKeyInput'); if(!input)return; const key=input.value.trim(); if(!key){ toast('API 키를 입력하세요.'); input.focus(); return; } db.settings.geminiApiKey=key; saveDB(); toast('Gemini API 키를 저장했습니다.'); render(); loadGeminiModels(false); }
function saveGeminiModel(model){ const m=GEMINI_MODELS.find(x=>(x.baseModelId||x.name.replace(/^models\//,''))===model); if(!m || !(m.supportedGeneration||[]).includes('generateContent')) return; db.settings.geminiModel=model; saveDB(); toast('AI 모델을 변경했습니다.'); }
...
function showRestoreChoice(x){
  const validReasons=new Set(REASONS.map(r=>r[0]));
  const cleanRecords=x.records.filter(r=>r.exam_id&&r.question_number&&TOPICS.some(t=>t.id===Number(r.topic_id))&&STATUS[r.status]&&Array.isArray(r.reasons)&&r.reasons.every(v=>validReasons.has(v))).map(r=>({...r,id:r.id||uid('rec_'),created_at:r.created_at||new Date().toISOString()}));
  ...
  pendingRestoreData={exams:cleanExams,records:cleanRecords};
  document.getElementById('modalRoot').innerHTML=`<div class="modal-backdrop"><div class="modal"><div class="modal-head"><h3>백업 데이터 복원</h3><button class="close-btn" onclick="closeModal()">×</button></div><div class="notice">${preview}</div><div class="field" style="margin-top:14px"><button class="secondary-btn full" onclick="mergeRestore()">기존 데이터를 유지하고 추가</button></div><div class="field"><button class="danger-btn full" onclick="replaceRestore()">기존 데이터를 모두 지우고 복원</button></div></div></div>`;
}
function mergeRestore(x=pendingRestoreData){
  if(!x)return toast('복원할 데이터가 없습니다.');
  ...
  let added=0; x.records.forEach(r=>{ const eid=idMap.get(r.exam_id); if(!eid)return; if(db.records.some(z=>z.exam_id===eid&&Number(z.question_number)===Number(r.question_number)))return; db.records.push({...r,id:uid('rec_'),exam_id:eid}); added++; }); saveDB();pendingRestoreData=null;closeModal();toast(`${added}개 기록을 추가했습니다.`);render(); }
function replaceRestore(x=pendingRestoreData){ if(!x)return toast('복원할 데이터가 없습니다.'); if(!confirm('기존 오답 기록이 모두 삭제되고 백업 파일의 데이터로 교체됩니다.\n\n정말 복원할까요?'))return; db={exams:x.exams,records:x.records,settings:{...DEFAULT.settings,...db.settings}}; saveDB();pendingRestoreData=null;closeModal();toast('백업 데이터로 복원했습니다.');render(); }
...
Object.assign(window,{render,openRecordForm,openExamPicker,openNewExamDialog,selectExam,filterExamChoices,createExamInline,toggleOtherReason,submitRecord,openRecordDetail,deleteRecord,closeModal,backupData,restoreData,mergeRestore,replaceRestore,resetData,setTheme,saveGeminiApiKey,clearGeminiApiKey,toggleApiKeyVisibility,loadGeminiModels,drawAllCharts});
render();
if(db.settings.geminiApiKey) setTimeout(()=>loadGeminiModels(false), 0);
