...
const DEFAULT = { exams:[], records:[], settings:{theme:'system', afterSave:'next'} };
let db = loadDB();
...
function loadDB(){ try { const x=JSON.parse(localStorage.getItem(KEY)); return x && x.exams && x.records ? {...DEFAULT,...x,settings:{...DEFAULT.settings,...x.settings}} : structuredClone(DEFAULT); } catch { return structuredClone(DEFAULT); } }
...
function renderAIPlaceholder(){
  ...
  return `<div class="hero-card"><h2>AI 학습 상담</h2><p>AI 기능은 현재 비활성화되어 있습니다. 아래 기록·통계 데이터는 준비해 두었으며, 나중에 AI를 연결하면 이 데이터를 기반으로 학습 상담을 할 수 있습니다.</p><button class="secondary-btn full" style="margin-top:14px" disabled>AI 분석하기 · 준비 중</button></div><div class="analysis-card"><h3>현재 기록</h3><p>AI에 연결하기 전에도 오답 분석은 정상적으로 사용할 수 있습니다.</p><div class="metric-row"><div class="metric"><b>${total}</b><span>전체 기록</span></div><div class="metric"><b>${counts.wrong}</b><span>틀림</span></div><div class="metric"><b>${counts.unsure+counts.guess}</b><span>헷갈림·찍음</span></div></div></div><div class="notice">AI가 숫자를 직접 세는 방식이 아니라, 앱이 먼저 계산한 단원·상태·이유·모의고사별 통계를 AI에 전달하는 구조로 설계되어 있습니다. 개선 방안의 자유 입력도 향후 AI가 참고할 수 있습니다.</div>`;
}
...
function renderSettings(main){
  ...
  <div class="settings-section"><div class="settings-title">앱 정보</div><div class="settings-card"><div class="settings-btn"><span>버전<small>1.0.0 · AI 기능은 아직 연결하지 않음</small></span></div><div class="settings-btn"><span>데이터 저장 방식<small>이 기기의 브라우저 localStorage에 저장됩니다.</small></span></div><button class="settings-btn" onclick="resetData()"><span style="color:var(--danger)">모든 데이터 삭제<small>모의고사와 오답 기록을 모두 삭제합니다.</small></span><b>›</b></button></div></div>
...
function backupData(){ const payload={app:'사문 오답 분석',version:1,exported_at:new Date().toISOString(),...db}; const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`사문_오답노트_${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(a.href); toast('백업 파일을 만들었습니다.'); }
...
Object.assign(window,{render,openRecordForm,openExamPicker,openNewExamDialog,selectExam,filterExamChoices,createExamInline,toggleOtherReason,submitRecord,openRecordDetail,deleteRecord,closeModal,backupData,restoreData,mergeRestore,replaceRestore,resetData,setTheme,drawAllCharts});
render();
